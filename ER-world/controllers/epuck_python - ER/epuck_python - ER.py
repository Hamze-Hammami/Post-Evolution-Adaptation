...
class Controller:
    def __init__(self, robot):        
        ...
        self.number_input_layer = 16 # 8 distance sensos + 8 light sensors 
        ...
        # Calculate the number of weights of MLP
        ...
        self.stage = "FORWARD"  # States: FORWARD, JUNCTION, TURNING, FINAL
    def detect_junction(self):
    

        front_proximity = max(self.inputs[15],self.inputs[8])  # Front sensors
        side_proximity = max( self.inputs[10], self.inputs[13])  # Side sensors
        
        # Junction detection conditions
        is_junction = (side_proximity < 0.05 and front_proximity <0.05)       
    ...
    def read_initial_light(self):
        ...
        light_values = self.inputs[0:8]
        # Calculate average light intensity
        avg_light = np.mean(light_values)
        
        return avg_light < 0.7  # Return True ifthere light detected

    def determine_turn_direction(self):
        if self.initial_light_state:
            return "RIGHT"  
        return "LEFT"      
    ...
    def sense_compute_and_actuate(self):
        ...
        # Store initial light reading if not already stored
        if self.initial_light_state is None:
            self.initial_light_state = self.read_initial_light()
            #this method for rewarding only it's only states 
        if self.stage == "FORWARD":
            if self.detect_junction():
                self.stage = "JUNCTION"
                self.has_reached_junction = True
            else:
                # Use MLP for forward navigation
                output = self.network.propagate_forward(self.inputs)
                self.velocity_left = output[0]
                self.velocity_right = output[1]
        
        elif self.stage == "JUNCTION":
            turn_direction = self.determine_turn_direction()
            self.stage = "TURNING"
            self.execute_turn(turn_direction)
            
        elif self.stage == "TURNING":
          
            if max(self.inputs[8:12]) > 0.05: 
              
                self.stage = "FINAL"
                
        elif self.stage == "FINAL":
            # Use MLP for final straight movement
            output = self.network.propagate_forward(self.inputs)
            self.velocity_left = output[0]
            self.velocity_right = output[1]

        # Apply motor velocities
        self.left_motor.setVelocity(self.velocity_left * 4)
        self.right_motor.setVelocity(self.velocity_right * 4)
